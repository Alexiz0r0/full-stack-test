create or replace NONEDITIONABLE PACKAGE BODY pkg_paciente AS
    ...
    BEGIN
        -- 1. Insertar el Paciente (asumiendo que id_paciente es IDENTITY)
        -- Usamos la cláusula RETURNING para obtener el ID generado en el momento
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK; -- Si algo falla, no se crea ni el paciente ni la historia
    ...
    PROCEDURE sp_actualizar(
        ...
        p_fecha_nacimiento IN DATE,
        p_sexo             IN CHAR,
        p_observaciones    IN CLOB -- Agregado parámetro de observaciones
    ) AS
    BEGIN
    ...
    BEGIN
        -- Gracias al "ON DELETE CASCADE" en la FK de historia_clinica,
        -- al borrar el paciente se borrará automáticamente su historia.
